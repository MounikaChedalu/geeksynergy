...
const MovieList = () => {
  ...
  return (
    ...
    <div className='info-div'>
      <h2>GEEKSYNERGY</h2>
      <button onClick={handleClick} className='info-button'>Company info</button>
          ...
          {movieData.map((movie, index) => (
            <li key={index} className='movie-item'>
              ...
              <div className='details-column'>
                ...
                <span>
                  <p>Mins | {movie.language} |
                {formatDate(movie.releasedDate)}</p>
                </span>
                <p className='blue-text'>{movie.pageViews} views|
                  <span>Votes</span> voted by{' '}
                  <span>1 people</span>
                </p>
